...
@property (assign, nonatomic) AFHTTPSessionManager *imageUrlManager;
@property (assign, nonatomic) AFHTTPSessionManager *loginManager;
@end
...
//    if (!_manager) {
//        AFHTTPSessionManager *manager = [AFHTTPSessionManager manager];
         AFHTTPSessionManager *manager = [[AFHTTPSessionManager alloc] initWithBaseURL:[NSURL URLWithString:YCH_URL_Server]];
...
//        [manager.requestSerializer setValue:@"application/json" forHTTPHeaderField:@"Content-Type"];
        NSString *authorization = [[NSUserDefaults standardUserDefaults] objectForKey:Authorization];
        if (authorization && authorization.length > 0) {
            [manager.requestSerializer setValue:authorization forHTTPHeaderField:@"Authorization"];
            NSLog(@"Authorization大于0时：%@",authorization);
        } else {
            [manager.requestSerializer setValue:nil forHTTPHeaderField:@"Authorization"];
            [UserCenter setIsLogin:NO];
             NSLog(@"Authorization小于0时：%@",authorization);
        }
        //配置响应序列化
...
- (AFHTTPSessionManager *)loginManager {
    ...
    if (!_loginManager) {
        //        AFHTTPSessionManager *manager = [AFHTTPSessionManager manager];
        AFHTTPSessionManager *loginManager = [[AFHTTPSessionManager alloc] initWithBaseURL:[NSURL URLWithString:YCH_URL_Server]];
        //默认解析模式
        //        manager.requestSerializer = [AFHTTPRequestSerializer serializer];
        loginManager.responseSerializer = [AFJSONResponseSerializer serializer];
        
        loginManager.requestSerializer = [AFJSONRequestSerializer serializer];
        
        loginManager.requestSerializer.stringEncoding = NSUTF8StringEncoding;
        loginManager.requestSerializer.timeoutInterval = TIMEOUT;
        //配置响应序列化
        loginManager.responseSerializer.acceptableContentTypes = [NSSet setWithArray:@[@"application/json", @"text/html", @"text/json", @"text/plain", @"text/javascript", @"text/xml", @"image/*", @"application/octet-stream", @"application/zip"]];
        _loginManager = loginManager;
    }
    for (NSString *key in headers.allKeys) {
        if (headers[key] != nil) {
            [_loginManager.requestSerializer setValue:headers[key] forHTTPHeaderField:key];
        }
    }
    //每次网络请求的时候，检查此时磁盘中的缓存大小，阈值默认是40MB，如果超过阈值，则清理LRU缓存,同时也会清理过期缓存，缓存默认SSL是7天，磁盘缓存的大小和SSL的设置可以通过该方法[ZBCacheManager shareManager] setCacheTime: diskCapacity:]设置
    [[ZBCacheManager shareManager] clearLRUCache];
...
#pragma mark - imageUrlManager
- (AFHTTPSessionManager *)imageUrlManager {
    [AFNetworkActivityIndicatorManager sharedManager].enabled = YES;
    if (!_imageUrlManager) {
        //        AFHTTPSessionManager *manager = [AFHTTPSessionManager manager];
        AFHTTPSessionManager *imageUrlManager = [[AFHTTPSessionManager alloc] initWithBaseURL:[NSURL URLWithString:YCH_URL_Server]];
        
        //默认解析模式
        imageUrlManager.requestSerializer = [AFHTTPRequestSerializer serializer];
        imageUrlManager.responseSerializer = [AFJSONResponseSerializer serializer];
        
//        imageUrlManager.requestSerializer = [AFJSONRequestSerializer serializer];
        
        imageUrlManager.requestSerializer.stringEncoding = NSUTF8StringEncoding;
        imageUrlManager.requestSerializer.timeoutInterval = TIMEOUT;
        //        [manager.requestSerializer setValue:@"application/json" forHTTPHeaderField:@"Accept"];
        [imageUrlManager.requestSerializer setValue:@"multipart/form-data" forHTTPHeaderField:@"Content-Type"];
        
//        NSString *authorization = [[NSUserDefaults standardUserDefaults] objectForKey:Authorization];
//        if (authorization && authorization.length > 0) {
//            [imageUrlManager.requestSerializer setValue:authorization forHTTPHeaderField:@"Authorization"];
//            NSLog(@"Authorization大于0时：%@",authorization);
//        } else {
//            //            [manager.requestSerializer setValue:nil forHTTPHeaderField:@"Authorization"];
//            //             NSLog(@"Authorization小于0时：%@",authorization);
//        }
        ...
        //配置响应序列化
        imageUrlManager.responseSerializer.acceptableContentTypes = [NSSet setWithArray:@[@"application/json", @"text/html", @"text/json", @"text/plain", @"text/javascript", @"text/xml", @"image/*", @"application/octet-stream", @"application/zip"]];
        _imageUrlManager = imageUrlManager;
    }
    for (NSString *key in headers.allKeys) {
        if (headers[key] != nil) {
            [_imageUrlManager.requestSerializer setValue:headers[key] forHTTPHeaderField:key];
        }
    }
    //每次网络请求的时候，检查此时磁盘中的缓存大小，阈值默认是40MB，如果超过阈值，则清理LRU缓存,同时也会清理过期缓存，缓存默认SSL是7天，磁盘缓存的大小和SSL的设置可以通过该方法[ZBCacheManager shareManager] setCacheTime: diskCapacity:]设置
    [[ZBCacheManager shareManager] clearLRUCache];
    return _imageUrlManager;
}
...
- (ZBURLSessionTask *)getWithUrl:(NSString *)url cache:(BOOL)cache params:(NSDictionary *)params progressBlock:(ZBGetProgress)progressBlock successBlock:(ZBResponseSuccessBlock)successBlock failBlock:(ZBResponseFailBlock)failBlock {
    ...
    session = [manager GET:url parameters:params headers:nil progress:^(NSProgress * _Nonnull downloadProgress) {
        ...
        if (responseObject) {
//            if ([responseObject[@"code"] longValue] == 500210) {
//                [UserCenter setIsLogin:NO];
//            }
            if ([responseObject[@"code"] longValue] == 200) {
                BOOL isValid = [self networkResponseManage:responseObject];
                if (successBlock && isValid) successBlock(responseObject);
            } else if ([responseObject[@"code"] longValue] == 500210) {
                [UserCenter setIsLogin:NO];
                 
            } else {
                failBlock(responseObject[@"msg"]);
            }
        }
...
- (ZBURLSessionTask *)uploadFileWithUrl:(NSString *)url fileData:(NSData *)data name:(NSString *)name fileName:(NSString *)fileName mimeType:(NSString *)mimeType progressBlock:(ZBUploadProgressBlock)progressBlock successBlock:(ZBResponseSuccessBlock)successBlock failBlock:(ZBResponseFailBlock)failBlock {
    __block ZBURLSessionTask *session = nil;
    AFHTTPSessionManager *manager = [self imageUrlManager];
    if (networkReachabilityStatus == AFNetworkReachabilityStatusNotReachable) {
...
//    }
    ...
    session = [manager GET:url parameters:nil headers:nil progress:^(NSProgress * _Nonnull downloadProgress) {
        ...
        if (successBlock) {
            ...
            NSData *imageData = [NSData dataWithContentsOfFile:downFileUrl];
